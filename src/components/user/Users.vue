  ...
  <div>
    ...
    <!--修改-->
     ...
     formitem 属性
         ...
         ref 一般写在el-form中 以作为验证表单使用 就是表单的引用 就是为了让其使用表单验证
         this.$ref.editFormRef .validate, resetField(),clearValidate()
        方法：
...
export default {
 ...
 // 验证邮箱的规则
    ...
    return {
      ...
      userlist: [],
      total: 0,
      addDialogVisible:false, //默认定义对话框是否显示
      // 添加用户的表单数据
      addForm: {
        username: '',
        password: '',
  ...
  methods: {
    ...
    // await的意思是等待请求完成后 把其值赋值到
    // 把返回的data 重命名为res
    // async 引入使得异步的操作更加的方便，async返回值是一个promise对象，因此可以使用then
    // 当函数执行的时候，一旦遇到await就会先返回，等到异步操作完成，再接着执行函数体内后面的内容。
    ...
    editUserInfo() {
        ...
        // $refs.editFormRef这个ref属性获取dom ref相当于一个id
